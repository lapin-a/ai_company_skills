...
- 기간 2010Q1~2026Q2(66분기, 2026-09-23 채택 — 이전은 2019Q1~), 연결재무제표, 보통주, 단위 원.
...
| `데이터 없음(보고서에 연결재무제표 없음)` | 국내 옛 양식(주로 2010~2012 분기·반기) 보고서 본문에 연결재무제표가 없는 기간. 값구분 `데이터 없음(연결 미수록)` (대표 결정 2026-09-22) |
...
18. **수집 기간 2010Q1~2026Q2(66분기)로 확대 — 결정 (대표, 2026-09-22), 채택 (가) 2026-09-23 (`ee724a0`).** 시험: 미국 라운드 31 · 국내 라운드 10. 2010~2012 국내 분기·반기는 보고서에 연결재무제표가 없어 `데이터 없음(연결 미수록)`으로 둔다(대표 결정 (1)). 국내 수집기는 OpenDART 원본 XML로 전환했고 라운드 1~14 회귀 변경 0. 상세 [us/logs/us-round31-log.md](us/logs/us-round31-log.md) 7절 · [kr/logs/round6-10-log.md](kr/logs/round6-10-log.md) 끝. 남은 일: 두 트랙 `WINDOW` 시작을 2010으로 바꾸고 전 라운드 재생성·채택.
    - **재생성 완료 (2026-09-23 01:11), 채택 완료.** 커버리지 표(kr·us coverage-log.md) 칸 수는 아직 30분기 기준 — 기획 2팀 재집계 필요. `WINDOW` 2010Q1~ 적용, 결과 `tmp_verify/w10/k2010/`·`u2010/`, 집계 `tmp_verify/w10/sum2010.py`.
