...
- **미국 트랙:** [us/coverage-log.md](us/coverage-log.md) — 미국 라운드 1~16, 미국 요약표·미확인 내역, 미국 전용 규칙, 미국 진행 중 사항
...
- 데이터셋: 국내 `kr/data/round1-dataset.csv` ~ `round14-dataset.csv` (14개, 11~14는 금융 항목 세트·`항목세트` 열 추가) · 미국 `us/data/us-round1-dataset.csv` ~ `us-round16-dataset.csv` (16개). 라운드별 요약은 같은 이름의 `-summary.json`.
- 공용 검증 스크립트: `tmp_verify/outliers_kr.py`(이상치·부호전환, 국내·미국 공용). 트랙별 검증 스크립트는 각 트랙 로그에 있다.

## 누적 요약

- 국내 + 미국 누적: **394개 기업**, 값 있는 칸 **126,995** · 데이터 없음 **2,851** · 미확인 **1,254**. (데이터셋 직접 집계, 2026-09-21 국내 금융 라운드 11~14 반영, 카카오뱅크 제외)
  - 재작성 반영 칸: 국내 710(비금융 449 · 금융 261) · 미국 709.
  - 데이터 없음 중 금융 "항목 미공시" 885칸(영업수익 290 · 순이자이익 296 · 순수수료이익 299)은 회사가 그 줄을 싣지 않은 경우다. 기록은 각 데이터셋 옆 `…-restated.csv` (아래 진행 중 사항 5번).
  - 미확인 내역: 국내 1(두산) · 미국 1,253. 미국 상세는 [us/coverage-log.md](us/coverage-log.md) "미확인 내역".
