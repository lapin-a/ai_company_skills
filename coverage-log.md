...
- **2010년대 백필 (2026-09-21 시작, 진행 중):** 수집 기간을 2010Q1부터로 넓히는 중. 코드 수정·표본 검증까지 끝났고 **데이터셋은 아직 2019Q1부터다.** 국내 [kr/logs/backfill-2010s-log.md](kr/logs/backfill-2010s-log.md) · 미국 [us/logs/us-backfill-2010s-log.md](us/logs/us-backfill-2010s-log.md)
...
- 기간 2019Q1~2026Q2, 연결재무제표, 보통주, 단위 원.
  - **2010Q1~2026Q2(66분기)로 넓히는 중 (2026-09-21).** 수집 코드(`WINDOW`)는 이미 2010Q1부터다. 데이터셋을 다시 만들면 한 기업당 기본 칸이 330 → 726이 된다. 그 전까지 요약표 숫자는 2019Q1~ 기준이다.
  - 2010년 국내 값은 회사에 따라 K-GAAP 또는 K-IFRS(조기도입)다. 2011년부터 K-IFRS. 기준 단절은 고치지 않고 기록만 한다(국내 백필 로그 5절).
...
| `데이터 없음(보고서에 해당 분기 연결재무제표 없음)` | **신설 제안(2026-09-21, 대표 승인 전).** 2014년 이전 옛 목차 보고서에 그 분기 날짜의 연결재무제표가 하나도 없을 때 (K-IFRS 도입 전 분기보고서). 값구분 `데이터 없음(연결 미수록)`. 국내 백필 로그 4절 |
...
16. **2010년대 백필 (2026-09-21 시작)** — 결정 필요 2건:
    - 새 결측 라벨 `데이터 없음(보고서에 해당 분기 연결재무제표 없음)` 승인 여부 (국내).
    - CIK가 바뀐 미국 회사(GOOGL·AVGO·DIS·MDT·ETN 등)에 라운드 4 MRVL 선례대로 옛 CIK를 이어 붙일지 (미국 백필 로그 4절).
    - 그 밖의 남은 일(국내 III장 연결재무제표·LG화학 재무상태표, 미국 상장 전 라벨·TSLA/MU 대차 실패, 전 라운드 재생성·회귀)은 트랙 백필 로그 "남은 일".
