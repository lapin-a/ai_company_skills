"""국내 회귀: 라운드 1~14를 원래 창으로 tmp_verify/w10/kreg/에 재생성 (원본은 건드리지 않는다). 라운드별 별도 프로세스."""
import os, subprocess, sys
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
C = """
import sys; sys.path.insert(0, "kr")
import collect_round2 as c2
c2.OUTDIR = "tmp_verify/w10/kreg/"
rnd = %d
if rnd >= 11:
...
for rnd in [int(a) for a in sys.argv[1:]] or range(1, 15):
    ...
    open(os.path.join(ROOT, "tmp_verify/w10/kreg/round%d.log" % rnd), "w", encoding="utf-8").write(p.stdout + p.stderr)
