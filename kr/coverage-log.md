...
- 라운드 11~14 상세: [round11-14-log.md](logs/round11-14-log.md) — 금융 18곳(금융 10항목, 카카오뱅크 제외), 금융 전용 파서 보정 6종(`FIN` 플래그)
...
- 라운드 11~14(금융 18곳, 카카오뱅크 제외): 값 5,992 · 데이터 없음 1,028(항목 미공시 885 포함) · 미확인 0 · 재작성 반영 261. 위 24,437·449는 라운드 1~10 기준이다.
...
### 이상치 집계 — 국내 (판정 규칙은 공통 파일)
  ...
  - **라운드 11~14 (금융, 2026-09-21):** 이상치 **153건**(계산값 21, 재확인 전) · 부호전환 289 ([round11-14-log.md](logs/round11-14-log.md) 6절).
...
2. ~~금융회사 수집~~ → **완료 (2026-09-21, 라운드 11~14).** 18곳 미확인 0 ([round11-14-log.md](logs/round11-14-log.md)). 카카오뱅크는 전 기간 연결재무제표 미해당이라 커버리지에서 뺐다(대표 결정, 같은 로그 9절).
   ...
   - 남은 것 ②: 계산값 이상치 21건 재확인은 아직 하지 않았다. `recheck_calc_kr.py`를 금융용으로 고쳐야 한다.
