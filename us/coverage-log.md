...
- 미국 라운드 31: [us-round31-log.md](logs/us-round31-log.md) — 5곳(BE·ILMN·P·LDOS·ROL), 미확인 10칸. **재작성 검증 실패 시 지배지분 자본만 섞이는 공용 코드 버그 47칸 발견, 결정 대기**
...
17. **재작성 검증 실패 칸의 지배지분 자본 섞임 47칸 — 고칠지 결정 필요 (2026-09-22 라운드 31에서 발견).** 대차 실패로 자산·부채·총자본은 원 공시로 되돌아가는데 지배지분 자본만 재작성값이 들어가 총자본 = 지배 + 비지배가 깨진다. 안: 저량 4항목을 한 묶음으로 되돌림 + 라운드 1~31 회귀. 상세 [us-round31-log.md](logs/us-round31-log.md) 5절
